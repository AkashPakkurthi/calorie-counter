...
from ..models import ActivityLog, User, WaterLog, WeightLog
from ..schemas import (
    ActivityIn,
    ActivityOut,
    WaterIn,
...
async def read_activity(
    ...
    return await get_activity(db, user.id, today_str() if date == "today" else date)
...
async def set_activity(
    ...
    result = await db.execute(
        select(ActivityLog).where(
            ActivityLog.user_id == user.id, ActivityLog.date == date
        )
    )
    row = result.scalar_one_or_none()
    if row is None:
        row = ActivityLog(user_id=user.id, date=date)
        db.add(row)
    row.walking_min = max(payload.walking_min, 0)
    row.tt_min = max(payload.tt_min, 0)
    await db.commit()
    return await get_activity(db, user.id, date)


@router.get("/water/{date}", response_model=WaterOut)
async def read_water(
    date: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(current_user),
):
    return await get_water(db, user.id, today_str() if date == "today" else date)
...
async def set_water(
    ...
    date = today_str() if date == "today" else date
    result = await db.execute(
        select(WaterLog).where(WaterLog.user_id == user.id, WaterLog.date == date)
    )
    row = result.scalar_one_or_none()
    if row is None:
        row = WaterLog(user_id=user.id, date=date)
        db.add(row)
    row.ml = max(payload.ml, 0)
    await db.commit()
    return await get_water(db, user.id, date)
...
async def log_weight(
    ...
    result = await db.execute(
        select(WeightLog).where(WeightLog.user_id == user.id, WeightLog.date == date)
    )
    row = result.scalar_one_or_none()
    if row is None:
        row = WeightLog(user_id=user.id, date=date)
        db.add(row)
